#!/usr/bin/env bash
# Tests for libexec/ai-doc-check
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SCRIPT="$REPO_ROOT/libexec/ai-doc-check"
cd "$REPO_ROOT"
...
test_usage() {
    local out
    out="$("$BASH_BIN" -c 'source libexec/ai-doc-check 2>/dev/null; usage' 2>&1 || true)"
...
test_excludes_generated() {
    "$BASH_BIN" -c '
        source libexec/ai-doc-check 2>/dev/null
