...
# Test suite for libexec/ai-search and critical libexec/ai-search-multi wiring.
# Must be invoked with CWD at the repo root (matches run-all-tests.sh's `cd`).
#
# Helpers:
...
SCRIPT="libexec/ai-search"
MULTI_SCRIPT="libexec/ai-search-multi"
...
run_search text "XYZZY_NO_MATCH_4f7a2b9c" libexec --fixed
...
AI_LANG=php run_search struct '$A' libexec --fixed
...
# cleanly if rg cannot be hidden from PATH in this environment. The query must
# appear in a tracked file (git grep only scans tracked content), so use a token
# from the committed README rather than an app-specific fixture term.
run_search_without rg text Agent .
...
# every tracked file. See lib/ai-search/dispatch.sh emit_results.
...
# -----------------------------------------------------------------------------
# Self-introspection (reachable in the default CI run) — covers the PHP->Bash
# introspection migration: --introspect emits the static contract without running
# a search, and --help renders the usage + mode discovery.
# -----------------------------------------------------------------------------
if command -v jq >/dev/null 2>&1; then
    printf '[introspect] self-introspection\n'
    set +e
    INTRO_JSON="$(AI_OUTPUT=json "$BASH_BIN" "$SCRIPT" --introspect 2>/dev/null)"
    INTRO_RC=$?
    set -e
    if [[ "$INTRO_RC" -eq 0 ]] && printf '%s' "$INTRO_JSON" | jq -e \
        '.schema == "ai.sh-introspect/v1" and .status == "ok" and .tool == "sh-introspect" and .meta.target_executed == false' >/dev/null; then
        printf '  PASS --introspect emits the static contract (not a search)\n'
    else
        printf '  FAIL --introspect static contract (rc=%s, json=%s)\n' "$INTRO_RC" "$INTRO_JSON" >&2
        exit 1
    fi
...
if ! command -v jq >/dev/null 2>&1; then
    printf '  PASS phase6 self-introspection tests skipped because jq is not installed\n'
else
    ...
    if printf '%s' "$HELP_OUT" | grep -qi 'modes'; then
        printf '  PASS --help surfaces mode discovery\n'
    else
        printf '  FAIL --help surfaces mode discovery\n' >&2
        exit 1
    fi
