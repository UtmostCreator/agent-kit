...
test_assert_inside_repo_pass() {
    (cd "$REPO_ROOT" && assert_inside_repo "$REPO_ROOT/lib/common.sh")
}
...
test_repo_relative_path() {
    local rel
    rel="$(cd "$REPO_ROOT" && repo_relative_path "$REPO_ROOT/lib/common.sh")"
    assert_eq "lib/common.sh" "$rel"
}
...
test_assert_relative_safe_pass() {
    (assert_relative_safe_path "lib/common.sh")
}
...
test_protected_normal() {
    ! path_matches_protected_pattern "lib/common.sh"
}
run_test "path_matches_protected_pattern: lib/common.sh → false" test_protected_normal
