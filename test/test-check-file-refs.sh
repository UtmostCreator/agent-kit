#!/usr/bin/env bash
# Tests for libexec/check-file-refs
set -euo pipefail
BASH_BIN="${BASH_BIN:-$(command -v bash)}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SCRIPT="$REPO_ROOT/libexec/check-file-refs"
...
# real external project (headless-cms) where check-file-refs's whole-repo
