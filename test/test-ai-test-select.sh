#!/usr/bin/env bash
# Tests for libexec/ai-test-select
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
SCRIPT="$REPO_ROOT/libexec/ai-test-select"
cd "$REPO_ROOT"
...
test_file_mode() {
    local out
    out="$(AI_LOG_DIR="$TMP/logs2" AI_EVENT_LOG="$TMP/logs2/ev.jsonl" "$BASH_BIN" "$SCRIPT" file lib/common.sh 2>/dev/null)"
    echo "$out" | jq -e '.input_files' >/dev/null
}
