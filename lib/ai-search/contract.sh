...
introspect_help_summary() {
    ...
    tool="$here/sh-introspect"
    # Static parse of the entrypoint header (usage/examples). Falls back silently
    # when the introspector is unavailable, so --help never breaks.
    [[ -x "$tool" ]] || return 0
    printf '\n---\nQuick contract (auto-generated by sh-introspect):\n\n'
    bash "$tool" --format=help "$here/ai-search" 2>/dev/null || true
