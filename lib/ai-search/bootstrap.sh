...
#
# Introspection target: the pure-Bash `libexec/sh-introspect` statically parses
#   ONLY the entrypoint file (it does not inline the sourced search modules), so
#   the human --help (usage/examples come from the entrypoint header) is complete
#   while the JSON --introspect flag list reflects the entrypoint alone.

# Early --help|-h delegate: render the introspector's help BEFORE sourcing any
...
# repo or rg/fd/ast-grep. If the introspector is unavailable it prints a minimal
# fallback rather than crashing.
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
    _here="$(CDPATH='' cd -- "$(dirname -- "${AI_SEARCH_ENTRYPOINT:-${BASH_SOURCE[0]}}")" && pwd)"
    _introspector="$_here/sh-introspect"
    if [[ -x "$_introspector" ]]; then
        exec bash "$_introspector" --format=help "$_here/ai-search"
    fi
    # Minimal fallback (introspector unavailable).
...
# Self-introspection: machine-readable contract for this script. Never executes
# search logic; delegates to the static introspector and replaces this process
# so no normal mode dispatch runs.
#
...
if [[ "${1:-}" == "--introspect" ]]; then
    ...
    exec env AI_OUTPUT=json bash \
        "$_here/sh-introspect" "$_here/ai-search"
fi

# shellcheck disable=SC1091
source "$(dirname "${AI_SEARCH_ENTRYPOINT:-${BASH_SOURCE[0]}}")/../lib/common.sh"
